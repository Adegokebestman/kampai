...
import { getUserInfo, getReadNotificationCount } from '../api/axios';
...
const Navbar = () => {
	...
	useEffect(async () => {
		...
		if (mounted) {
			...
			setNotificationCount(user.userInfo.notifications.length);
		}
	...
	return (
		<div className=' flex mt-2 justify-around p-2 md:ml-6 md:mr-6 '>
			...
			<NavButton
				className='inline-block pr-8 pl-8'
				title='Menu'
        ...
        /> */}
			...
			>
				<span class='relative inline-flex items-center text-sm font-medium text-center text-white'>
					<img
						className='w-6 h-6 md:w-12 mr-2 md:h-8'
						src={notification}
						alt={notification}
					/>
